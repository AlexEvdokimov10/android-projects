...
export class TestHttpPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get('http://localhost/api/?a=10&adasda=1000').subscribe(data=>{
      console.log(data);
    });
    this.http.post('http://localhost/api/',{
      nameOfType:'gold fish',
      amount:2
  ...
  loadData(refresher){
    (this.http.get('https://jsonplaceholder.typicode.com/posts').pipe(
      map((res: Array<any>)=>res.filter(row=>row.id>this.postCount && row.id<this.postCount+this.postStep))).subscribe(
      data=>{
